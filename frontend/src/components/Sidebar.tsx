const Sidebar = ({ currentView, onViewChange, userEmail, onLogout, onNavigate }: {
    currentView: string;
    ...
    return (
        <aside style={{ width: '280px', minWidth: '280px', padding: '2rem', borderRight: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', gap: '2rem', height: '100%', overflowY: 'auto' }}>
            ...
            <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                <button className={`nav-pill ${currentView === 'dashboard' ? 'active' : ''}`} onClick={() => go('dashboard')}>
                ...
                <button className={`nav-pill ${currentView === 'entities' ? 'active' : ''}`} onClick={() => go('entities')}>
            ...
            <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                <button className={`nav-pill ${currentView === 'regulatory-radar' ? 'active' : ''}`} onClick={() => go('regulatory-radar')}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><path d="M12 2a14.5 14.5 0 0 0 0 20 14.5 14.5 0 0 0 0-20" /><path d="M2 12h20" /></svg> Regulatory Radar</button>
                <button className={`nav-pill ${currentView === 'obligation-mapper' ? 'active' : ''}`} onClick={() => go('obligation-mapper')}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4" /><polyline points="16 17 21 12 16 7" /><line x1="21" y1="12" x2="9" y2="12" /></svg> Obligation Mapper</button>
                <button className={`nav-pill ${currentView === 'evidence-vault' ? 'active' : ''}`} onClick={() => go('evidence-vault')}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg> Evidence Vault</button>
                <button className={`nav-pill ${currentView === 'supplier-engine' ? 'active' : ''}`} onClick={() => go('supplier-engine')}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /><polyline points="22,6 12,13 2,6" /></svg> Supplier Engine</button>
                <button className={`nav-pill ${currentView === 'audit-workspace' ? 'active' : ''}`} onClick={() => go('audit-workspace')}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><polyline points="10 9 9 9 8 9" /></svg> Audit Workspace</button>
            </nav>
